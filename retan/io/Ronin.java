...
public class Ronin extends RetanPlayer {
...
//                sk.add(new RetanSkill(x + 25 - size / 2, y + 25 - size / 2, tdd, H, size, dam, Color.WHITE));
                sk.add(new RetanSkill(x + 25 - size / 2 + k, y + 25 - size / 2, tdd, H, size, dam, lx));
                sk.add(new RetanSkill(x + 25 - size / 2 - k, y + 25 - size / 2, tdd, H, size, dam, lx));
                sk.add(new RetanSkill(x + 25 - size / 2, y + 25 - size / 2 + k, tdd, H, size, dam, lx));
                sk.add(new RetanSkill(x + 25 - size / 2, y + 25 - size / 2 - k, tdd, H, size, dam, lx));
                sk.add(new RetanSkill(x + 25 - size / 2 + k2, y + 25 - size / 2 + k2, tdd, H, size, dam, lx2));
                sk.add(new RetanSkill(x + 25 - size / 2 - k2, y + 25 - size / 2 - k2, tdd, H, size, dam, lx2));
                sk.add(new RetanSkill(x + 25 - size / 2 - k2, y + 25 - size / 2 + k2, tdd, H, size, dam, lx2));
                sk.add(new RetanSkill(x + 25 - size / 2 + k2, y + 25 - size / 2 - k2, tdd, H, size, dam, lx2));
    ...
    public void xoay() {
        for (int i = 0; i < sk.size(); i++) {
            if (sk.get(i).color == lx) {
                sk.get(i).color = lx2;
            } else {
                if (sk.get(i).color == lx2) {
                    sk.get(i).color = lx;
                }
            }
        }
...
- Kỹ năng [F]&[1]: Bão tập kích (Cơ động cường hóa)
+ bắn một viên đạn tầm xa nếu trúng kẻ địch sẽ tích 1 tụ bão.
